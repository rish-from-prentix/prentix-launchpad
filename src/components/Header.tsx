  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border/50">
      <nav className="container mx-auto flex items-center justify-between h-16 md:h-20">
        ...
        <a href="#" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-bold text-foreground">Prentix.ai</span>
        </a>

        {/* Desktop Navigation */}
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map(link => <a key={link.label} href={link.href} className="nav-link text-sm font-medium">
              {link.label}
            </a>)}
        </div>
        ...
        <div className="hidden md:block">
          <button onClick={onOpenModal} className="btn-primary text-sm font-semibold">START YOUR INTERNSHIP
 
NOW</button>
        </div>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-background border-b border-border animate-fade-in">
          <div className="container py-6 flex flex-col gap-4">
            {navLinks.map(link => <a key={link.label} href={link.href} className="nav-link text-lg font-medium py-2" onClick={() => setMobileMenuOpen(false)}>
                {link.label}
              </a>)}
            <button onClick={() => {
